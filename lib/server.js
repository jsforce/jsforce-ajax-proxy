...
app.get('/kbsearch/', cors(corsOptions), async function (req, res) {
	...
	var raw = JSON.stringify(qrySet(qryParams.currCat, qryParams.srhTxt, categories, tags, categoryMatch, tagMatch, filterAndOr));
    var requestOptions = {
        method: 'POST',
        headers: {
			"Authorization": "Bearer " + qryParams.token,
...
app.get('/questionfieldfetch/', cors(corsOptions), async function (req, res) {
	...
	if(qryParams.objectApiName && qryParams.fieldSetApiName){		
		...
		let response;
		response = await fetch(qryParams.instanceURL+"/services/apexrest/avnio/QueFieldSets", requestOptions);
		if(response.status === 200){
			response = await response.text();
			res.send(response);
		}
...
function qrySet(currCat, srhText, categories, tags, categoryMatch, tagMatch, filterAndOr){
    let value = 0;
    value = currCat;
    let requestBody = {};
